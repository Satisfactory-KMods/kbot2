...
import { channelToSelection, modsToSelection, rolesToSelection } from '@lib/selectConversion';
...
export default function useSelection() {
	...
	const bothChannelOptions = useMemo(
		() => channelToSelection(forumChannels).concat(channelToSelection(textChannels)),
		[forumChannels, textChannels]
	);
	const modOptions = useMemo(
		() => modsToSelection(mods.filter((e) => guildData.options.ficsitUserIds.includes(e.creator_id))),
		[mods, guildData.options.ficsitUserIds]
	);
