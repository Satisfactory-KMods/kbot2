...
const Component: FC = () => {
	...
	const OnSubmit = async (event: FormEvent<HTMLFormElement>) => {
		...
		if (login !== undefined && password !== undefined) {
			...
			setIsLoading(true);
			const Response = await tRPC_Public.login
				.mutate({
					login,
					password,
					stayLoggedIn
	...
	return (
		<>
			<h1 className='text-center text-xl font-bold leading-tight tracking-tight text-gray-900 text-white md:text-2xl'>
			...
			<form className='space-y-4' action='#' onSubmit={OnSubmit}>
				<TextInput
					...
					placeholder='Discord id or login name'
					ref={loginRef}
					helperText={
						inputError[0] ? (
							<>
