...
export function channelToSelected<T extends DiscordTextChannel | DiscordForumChannel>( channels : T[], selectedIds : string[] ) : MultiValue<OptionSelection<string>> {
	return channelToSelection( channels ).filter( e => selectedIds.includes( e.value ) );
}

export function channelToSelectedSingle<T extends DiscordTextChannel | DiscordForumChannel>( channels : T[], selectedId : string ) : SingleValue<OptionSelection<string>> {
