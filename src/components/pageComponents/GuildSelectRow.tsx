...
const GuildSelectRow: FC<IGuildSelectRowProps> = ({ guild }) => {
	...
	return (
		<li className='p-0'>
			<button
				className='flex w-full items-center space-x-4 px-4 py-3 hover:bg-gray-700 sm:py-4'
				onClick={() => navigate(`/guild/${guild.guildId}/`)}>
				<div className='shrink-0'>
					<img className='h-12 w-12 rounded-full' src={guild.guildData.iconURL || '/images/invalid.png'} alt={guild.guildData.name} />
				</div>
				<div className='min-w-0 flex-1 text-left'>
					<p className='truncate font-medium text-gray-900 dark:text-white'>{guild.guildData.name}</p>
