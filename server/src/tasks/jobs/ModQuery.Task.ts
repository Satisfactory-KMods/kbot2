...
import { BuildStringGroup, GroupToString } from '@server/lib/bot/stringGrouper.lib';
							...
							if (!guild.options.blacklistedMods?.includes(mod.mod_reference)) {
								...
								if (modDocument.hash !== mod.versions[0].hash) {
									...
									if (!mod.hidden || (mod.hidden && guild.options.modsAnnounceHiddenMods)) {
											...
											fields: [
												{
													name:
														mod.versions[0].changelog !== ''
															? mod.versions[0].changelog.split(/\r?\n/)[0].substring(0, 255)
															: '...',
										...
										const role = await guildClass.role(
											guild.options.RolePingRules?.find((r) => r.modRefs.includes(mod.mod_reference))?.roleId ||
												(guild.options.defaultPingRole?.length > 0 ? guild.options.defaultPingRole : '0')
										);
										let roleId = '0';
										if (role) {
											roleId = role.id;
										...
										if (embed && annoucementChannel) {
											const msg = await annoucementChannel.send({
												embeds: [embed],
												content: `${
													roleId === '0' ? '@everyone' : '<@&' + roleId + '>'
												} new mod update has been released!\n\n`
											});
