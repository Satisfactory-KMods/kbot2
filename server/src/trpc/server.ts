import { MW_Auth, MW_AuthGuild, MW_AuthGuild_Leg } from '@server/lib/Express.Lib';
						...
						if (annoucementChannel) {
							...
							if (embed) {
								await annoucementChannel.send({
									embeds: [embed],
									content: `${patreonOptions.pingRoles
										.map((e) => `<@&${e}>`)
										.join(', ')}\n\n${patreonOptions.patreonReleaseText.replaceAll(
										'{changelogmessage}',
										thread.id.toString()
									)}`.substring(0, messageTextLimit - 1)
								});
							}
						}
					}
