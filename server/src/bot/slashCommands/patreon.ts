...
const command = new SlashCommandBuilder()
	.setName( "patreon" )
	.setDescription( "Create or get your patreon token to download dev builds!" );
	...
	try {
		...
		if ( interaction.inGuild() && guild?.IsValid ) {
			...
			if ( interaction.member.roles instanceof GuildMemberRoleManager && interaction.member.permissions instanceof PermissionsBitField ) {
				...
				if ( roleTest || interaction.member.permissions.has( PermissionFlagsBits.Administrator ) || interaction.member.permissions.has( PermissionFlagsBits.BanMembers ) ) {
