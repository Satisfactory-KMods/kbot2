...
import { BC, SystemLib_Class } from '@server/lib/System.Lib';
import { ActivityType, Client, CommandInteraction, GatewayIntentBits, Partials, REST, Routes, SlashCommandBuilder } from 'discord.js';
...
global.DiscordBot = new Client({
	intents: Object.entries(GatewayIntentBits)
		.map((R) => parseInt(R[0]))
		.filter((R) => !isNaN(R)),
						...
						for (const File of fs.readdirSync(path.join(__dirname, Dir))) {
							...
							if (Dir === 'slashCommands') {
								...
								if (!Options.find((x) => x.command.name === Option.command.name)) {
									...
									SystemLib.LogError(
										'bot',
										`Gather slash command FAILED reason DUPLICATED:${BC('Cyan')}`,
										`/${Option.command.name}`,
										path.join(Dir, File)
									);
								}
