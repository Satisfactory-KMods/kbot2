import { and, desc, eq, inArray, isNull, sql } from '@kmods/drizzle-pg';
...
export default defineEventHandler(async (event) => {
	...
	return await db
		...
		.where(
			and(
				inArray(scModAuthors.user_id, userIds),
				showHidden ? undefined : eq(scModCache.hidden, false)
			)
		)
		.then((mods) => {
			return {
